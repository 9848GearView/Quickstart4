...
@Autonomous(name = "BlueBigV2", group = "Examples")
@Configurable
public class BlueBigV2 extends OpMode {
    ...
    public static class Paths {
        ...
        private Pose startPosition = new Pose(20,124, Math.toRadians(323) );
        private Pose launchPosition = new Pose(53, 90, Math.toRadians(320));
        private Pose intake1Start = new Pose(53, 93, Math.toRadians(180));
        private Pose intake1End = new Pose(26, 93, Math.toRadians(180));
        private Pose intake2Start = new Pose(53, 67, Math.toRadians(180));
        private Pose intake2End = new Pose(17, 67, Math.toRadians(180));
        private Pose intake2Curve = new Pose(57, 60, Math.toRadians(180));
        private Pose intake3Start = new Pose(53, 21, Math.toRadians(180));//need real x & y
        private Pose intake3End = new Pose(16, 21, Math.toRadians(180));//need real x & y
        private Pose park = new Pose(23, 84, Math.toRadians(270));
    ...
    public int autonomousPathUpdate() {
        switch(pathState) {
            case 0:// Starting Pos to Launch 1
                timer.schedule(new ShootAuto(.735), 0);
            ...
            case 1: //launch 1 to intake 1 start
                if(!follower.isBusy() && pathTimer.getElapsedTimeSeconds() > 6.85){
                    pathTimer.resetTimer();
                    follower.followPath(paths.intake1S, .6,true);
                    setPathState(2);
                }
                break;
            case 2: //intake 1 start to intake 1 end
                if(!follower.isBusy()) {
                    timer.schedule(new IntakeAuto(1.0), 0);
                    timer.schedule(new IntakeAuto(0), 2700);
                    follower.followPath(paths.intake1E, .4, true);
                    setPathState(3);
                    pathTimer.resetTimer();
                }
                break;
            case 3: //intake 1 end to launch 2
            ...
            case 4: //launch 2 to intake 2 start
                ...
                break;
            case 5: //intake 2 start to intake 2 end
                if(!follower.isBusy()) {
                    timer.schedule(new IntakeAuto(1.0), 0);
                    timer.schedule(new IntakeAuto(0), 2700);
                    follower.followPath(paths.intake2E,.45,true);
                    setPathState(6);
                }
                break;
            case 6: //intake 2 end to launch 3
                if(!follower.isBusy() && pathTimer.getElapsedTimeSeconds() > 2.7) {
                    pathTimer.resetTimer();
                    timer.schedule(new ShootAuto(.735), 0);
                    timer.schedule(new GateAuto(1), 3500);
